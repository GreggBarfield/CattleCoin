...
import { toCents, dollars } from "./routeHelpers.js";

// Shared real-ledger computation for one investor's money picture: what they
// paid in for each herd, where each herd stands, and what they have been paid
// or are still owed from a sale. Used by GET /api/my-money (the investor's own
// full page) and by GET /api/investors/:slug/portfolio|holdings (the Dashboard
// and lot pages, which used to show fabricated numbers - see
// step-dashboard-real-numbers.md). Identity is always the caller's own userId;
// nothing here reads another investor's data.
//
...
export async function getInvestorMoney(userId) {
